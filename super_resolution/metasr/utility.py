...
class checkpoint():
    ...
    def save_results(self, filename, save_list, scale, scale2):
        filename = '{}/results/{}_noisex{}_srx{}_'.format(self.dir, filename, scale, scale2)
        postfix = ('SR', 'LR', 'HR')
...
def calc_psnr(sr, hr, scale, rgb_range, benchmark=False):
    ...
    if benchmark:
        ...
        if diff.size(1) > 1:
            ...
            diff = diff.sum(dim=1, keepdim=True) # transfer r, g, b to Y channel
    else:
        shave = scale + 6
    #shave = int(shave)
    import math
